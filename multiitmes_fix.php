...
<head>
	...
	function order_remove(orderid,vendor,tracknum)
	{		
		$("#submit_"+orderid).attr("disabled","disabled");
		
		$.post("ajax_taobao_sold_track.php?action=remove", { orderid: orderid, vendor: vendor, tracknum: tracknum }, function(data) {order_remove_callback(data)} );

	}
	function order_remove_callback(data)
	{
...
$DateEnd = new DateTime(date("Y-".($month+1)."-01 00:00:00", time()));
$FromDate = $Date->modify('-1 day')->format('Y-m-d 00:00:00');
$ToDate = $DateEnd->modify('-1 day')->format('Y-m-d 00:00:00');
$Today = new DateTime(date("Y-m-d 00:00:00", time()));
if ($DateEnd->modify('-1 day') > $Today)
{
	$ToDate = $Today->format('Y-m-d 00:00:00');
}
...
while ($nextpage)
{
	...
	foreach ($respTrades->trades->trade as $trade)
	{
		...
		$arrItems = array();
		$itemnum = count($trade->orders->order);

		if ($itemnum > 1 && isset($TaobaoOrders["$OrderNumber"]))
		{
			...
			$query = "SELECT SOrderID, PriceID, PSS_Item.ItemID AS ItemID, LegoID FROM  `PSS_SOrder` INNER JOIN PSS_Price ON SOrderID = LinkID INNER JOIN PSS_Item ON PSS_Price.ItemID = PSS_Item.ItemID WHERE `OrderNumber` = '".$mysqli->real_escape_string($OrderNumber)."' AND TYPE = 'Sold' LIMIT 0 , 30;";
			$result = $mysqli->query($query);
			...
			while ($row = $result->fetch_array(MYSQLI_ASSOC))
			{
				$SOrderID = $row["SOrderID"];
				$DBItem = new stdclass();
				$DBItem->{'PriceID'} = $row["PriceID"];
				$DBItem->{'ItemID'} = $row["ItemID"];
				$DBItem->{'LegoID'} = $row["LegoID"];
				array_push($arrDBItems, $row["LegoID"]);
			}
			$DBItems = join(",",$arrDBItems);
			...
			echo "<tr id=\"row_".$OrderNumber."\"><td>".$OrderTime."</td><td><a href=\"http://trade.taobao.com/trade/detail/trade_item_detail.htm?bizOrderId=".$OrderNumber."\">".$OrderNumber."</a></td><td>".$Items."(".$DBItems.")</td><td>".$ShippingTime."</td><td>".$Vendor.":".$TrackNum."</td><td><input type=\"text\" id=\"weight_".$OrderNumber."\" name=\"weight_".$OrderNumber."\" size=\"4\" value=\"".$Weight."\" onkeydown=\"enterEventHandler();\" onclick=\"select_focus(this.id);\">kg</td><td><input type=\"button\" id=\"submit_".$OrderNumber."\" value=\"删除\" onclick=\"order_remove('".$OrderNumber."','".$Vendor."','".$TrackNum."');\"></td><td id=\"message_".$OrderNumber."\"></td></tr>";
