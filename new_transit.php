...
.order_div ul li a { 
    ...
    function calc_shipping_total_byweight()
    {
		...
		$("input[name^='weight_']").each( function()
		{ 
		...
		$("input[name^='shippingfee_']").each( function() 
		{
			...
			var item_price = $("input[name='weight_"+item_id+"']").val();
	...
	function all_selected()
	{
    	...
    	$.post("ajax_new_transit.php?action=query_items", { ids: idstr}, function(data) {all_selected_callback(data)} );
	}
	function all_selected_callback(data)
	{
		$("#item_list").html(data);
		calc_shipping_total_byweight();
	}
	function search()
	{
		...
		if (search_text == "")
		{
			$.get("ajax_new_transit.php?action=list_order", null, function(data) {search_callback(data)} );
		}
		else if (search_type == "legoid")
		{
			$.get("ajax_new_transit.php?action=list_order", { legoid: search_text} , function(data) {search_callback(data)} );
		}
		else if (search_type == "orderid")
		{
			$.get("ajax_new_transit.php?action=list_order", { orderid: search_text} , function(data) {search_callback(data)} );
		}
	...
	function ajax_form_submit()
	{
    	...
    	$.post("ajax_new_transit.php?action=new_transit", postdata, function(data) {ajax_form_submit_callback(data)} );
	}
	...
	function check_shipping_orderid()
    {
    	...
    	$.get("ajax_new_transit.php?action=query_order", { oid: orderid}, function(data) {check_shipping_orderid_callback(data);} );
    	if ($("#Vendor").val() == "UPS")
    	{
...
<select name="ShippingTo" />
  ...
  <option value="LEMO-BJ04" selected>LEMO JYHF-2-1-2102</option>
</select>
</span>
<br />
<span>转运单号:<input type="text" size="27" id="RefID" name="RefID" /></span>
<br />
<span>承运方:
...
<span>运单号:<input type="text" size="20" name="ShippingOrderNumber" onblur="check_shipping_orderid();"/></span><span id="shipping_order_text"></span>
<br />
...
<br />
<span>总金额:<input type="text" size="7" id="total_amount" onblur="calc_shipping_total_byweight();"/>
<select id="Rate" name="Rate" onchange="calc_shipping_total_byweight();">
...
<table>
<thead><tr><th>LEGO编号</th><th>重量</th><th>均分运费</th></tr></thead>
